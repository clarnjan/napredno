...
class HospitalSystem<T extends Patient> {
    ...
    public void printResults() {
        ...
        PatientService<T> findAllWithDoctor= (patients, filter) -> (ArrayList<T>) patients.stream().filter(x-> x.getDoctor().equals(filter)).collect(Collectors.toList());

        PatientService<T> countAllWithGenderAndAgeLessThanSixty= (patients, filter) -> (ArrayList<T>) patients.stream().filter(x-> x.getAge()<60 && x.getGender().equals(filter)).collect(Collectors.toList());

        PatientService<T> findAllWithNameSorted= (patients, filter) -> (ArrayList<T>) patients.stream().filter(x-> x.getName().equals(filter)).sorted(Comparator.comparing(x->x.getCode())).collect(Collectors.toList());
